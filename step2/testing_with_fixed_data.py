...
def showX(X, alpha, rows=1):
    ...
    # print(filenames_5[idx]+'.jpg')
    Image.fromarray(target_image).save(os.path.join(target_dir, filenames_5[idx].split("/")[-1]))
...
netGA = load_model('step2/model/netG1556383922.122271.h5')
#netDA = load_model('./models/netD1526878347.025762.h5')

real_A, fake_B, rec_A, cycleA_generate, alpha_A = cycle_variables(netGA)

data = "step2/input/test"
train_A = load_data('{}/1/*.jpg'.format(data))

filenames_5 = load_data('{}/5/*.jpg'.format(data))
# assert 'step2/input/test/5/955.jpg' in filenames_5

out_root_dir = "step2/output"
origin_dir = os.path.join(out_root_dir, "original")
target_dir = os.path.join(out_root_dir, "target")
output_dir = os.path.join(out_root_dir, "result")
mask_dir = os.path.join(out_root_dir, "mask")
