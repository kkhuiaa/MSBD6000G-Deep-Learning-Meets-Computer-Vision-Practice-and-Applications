...
print('backend!!!!!!!!!!!', K.tensorflow_backend._get_available_gpus())
...
def read_image(fn):
    ...
    fn_y_j = np.random.choice(filenames_1)
    while (fn_y_j.split('/')[-1] == fn_y_i.split('/')[-1]):
        fn_y_j = np.random.choice(filenames_1)
    if isRGB:
...
def showX(X, rows=1, result_dir=None, gen_iterations=''):
    ...
    Image.fromarray(int_X).save(os.path.join(result_dir, str(time.time())+ '_' + str(gen_iterations) + '.jpg'))
    #display(Image.fromarray(int_X))

def showG(A, result_dir=None, gen_iterations=''):
    def G(fn_generate, X):
        r = np.array([fn_generate([X[i:i+1]]) for i in range(X.shape[0])])
        return r.swapaxes(0,1)[:,:,0]        
    rA = G(cycleA_generate, A)
    arr = np.concatenate([A[:,:,:,:3], A[:,:,:,3:6], A[:,:,:,6:9], rA[0], rA[1]])
    showX(arr, 5, result_dir=result_dir, gen_iterations=gen_iterations)
...
def plot_error(errGA_list, errDA_list, errCyc_list, plot_path=None, plot_file_name=None, plot_directory=None):
    df0 = pd.DataFrame({'iteration': range(1, len(errGA_list) + 1), 'error': errGA_list, 'type': ['errGA']*len(errGA_list)})
    df1 = pd.DataFrame({'iteration': range(1, len(errDA_list) + 1), 'error': errDA_list, 'type': ['errDA']*len(errDA_list)})
    df2 = pd.DataFrame({'iteration': range(1, len(errCyc_list) + 1), 'error': errCyc_list, 'type': ['errCyc']*len(errCyc_list)})
    ax = sns.lineplot(x='iteration', y="error", hue="type", style="type", markers=False, dashes=False, data=df0.append(df1).append(df2), err_style=None)
    sns.set_style('whitegrid')
    ax.legend(loc='upper right')
    plt.show()
    plt.tight_layout()
    plt.savefig(plot_path, dgi=400, bbox_inches='tight')
    plt.close()

if __name__ == '__main__':
    result_dir = 'step2/result'
    error_dir = 'step2/result/error'
    model_dir = 'step2/model'
    for directory in [result_dir, model_dir, error_dir]:
        if not os.path.exists(directory):
            os.makedirs(directory)
    data_dir = 'step2/input/train'
    train_A = load_data('{}/data/*.jpg'.format(data_dir))
    filenames_1 = load_data('{}/1/*.jpg'.format(data_dir)) #target
    filenames_5 = load_data('{}/5/*.jpg'.format(data_dir)) #model
    ...
    while gen_iterations < iteration_num:
        ...
        errDA  = netD_train([A])[0]
        errDA_list.append(errDA)
        # errDA_sum +=errDA[0]

        # epoch, trainA, trainB = next(train_batch)
        errGA, errCyc = netG_train([A])
        errGA_list.append(errGA)
        errCyc_list.append(errCyc)
        # errGA_sum += errGA
        # errCyc_sum += errCyc
        gen_iterations+=1
        t2 = time.time()
        if (gen_iterations%5==0) | (gen_iterations<50):
            # print(errDA, errGA, errCyc)
            print('gen_iterations:', gen_iterations, ' with required time per iter:', t2-t1)
            print('[%d/%d][%d] Loss_D: %f Loss_G: %f loss_cyc: %f'
            % (epoch, niter, gen_iterations, errDA, errGA, errCyc), time.time()-t0)
            # % (epoch, niter, gen_iterations, errDA_sum/display_iters,
            #    errGA_sum/display_iters, errCyc_sum/display_iters), time.time()-t0)

        if (gen_iterations%display_iters==0) | (gen_iterations==iteration_num) | (gen_iterations==1):
            _, A = train_batch.send(10)
            showG(A, result_dir=result_dir, gen_iterations=gen_iterations)
            plot_error(errGA_list, errDA_list, errCyc_list, plot_path=os.path.join(error_dir, str(time.time()) + '_' + str(gen_iterations) + '.jpg'))
        if ((gen_iterations%model_iters==0) | (gen_iterations==iteration_num)) & (gen_iterations >= model_iters):   
            netGA.save(os.path.join(model_dir, 'netG' + str(time.time()) + '_' + str(gen_iterations) + '.h5'))
            netDA.save(os.path.join(model_dir, 'netD' + str(time.time()) + '_' + str(gen_iterations) + '.h5'))
